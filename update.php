...
if ($stagiaire) {
  ...
  exit('<p>Ce stagiaire est introuvable. 😓');
}
?>
      ...
      <form action="traitement.php" method="POST" class="col-md-6 offset-md-3">
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="civilite" aria-label="civilité" value="<?= $stagiaire['civiliteStagiaire'] ?>">
            <?php
            foreach (["Mme" => "Madame", "M." => "Monsieur"] as $civilite => $libelle) {
              echo '<option value="' . $civilite . '"' .
                ($civilite === $stagiaire['civiliteStagiaire'] ? ' selected="selected"' : '')
                . '>' . $libelle . '</option>';
            }
            ?>
          </select>
          ...
          <select class="form-select" name="ville" aria-label="ville">
            ...
            foreach ($villes as $ville) {
              echo '<option value="' . $ville['idVille'] . '"' .
                ($ville['idVille'] === $stagiaire['idVille'] ? ' selected="selected"' : '')
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="formation" aria-label="formation" value="<?= $stagiaire['titreFormation'] ?>">
            ...
            foreach ($formations as $formation) {
              echo '<option value="' . $formation['idFormation'] . '"' .
                ($formation['idFormation'] === $stagiaire['idFormation'] ? ' selected="selected"' : '')
                . '>' . $formation['titreFormation'] . '</option>';
            }
            ?>
          </select>
        </div>
        ...
        $dbh = null;
        ?>
        <button type="submit" class="btn btn-primary" name="update">Créer</button>
      </form>
    </div>
  </section>
